...
In the specific implementation of TMD of this software package, a constraint is applied on the distance $x$ between two subsets of atoms, moving them with constant velocity $v$ from an initial state $(x)$ along the pulling coordinate towards a target conformation. This constraint,
...
$$
\begin{align}
\mathrm{e}^{-\beta\Delta G(x)} = \left< \mathrm{e}^{-\beta W(x)} \right>. 
\end{align}
$$
...
$$
\begin{align}
\Delta G(x) = \left< W(x) \right> -\frac{\beta}{2}\left<\delta W(x)^2\right> + \dots
\end{align} 
$$
...
$$	\left<W_{\text{diss}}(x)\right> = \frac{\beta}{2}\left< \delta W(x)^2 \right> \; .
$$
...
Here, the memory-free Langevin equation models the dynamics projected to a small set of coordinates, here the motion of the distance $s$, in terms of the mean force $\frac{\partial G}{\partial x}$, friction ${\it{\Gamma}}$ and fluctuations $\xi$, as well as the applied constraint force, as

$$	
\begin{align}
m \ddot{x}= - \frac{\partial G}{\partial x} - {\it{\Gamma}}(x)\,\dot{x} + \xi(t) + f(t) \quad ( = 0 ).
\end{align}
$$
...
When considering an ensemble of TMD trajectories instead of a single one, the stochastic term in Eq. (3) cancels out, $\left< \xi \right>=0$. Together with $\dot{x}=v$, integrating on both sides from the initial state $x_0$ to final state $s$ gives

$$
\begin{align}
	\Delta G(x) &= \int_{x_0}^{x}  \text{d}x' \left<f(x')\right> - v \int_{x_0}^{x}  \text{d}x' \; {\it{\Gamma}}(x')  
\end{align}
$$
...
$$
\begin{align*}
	\left<W_\text{diss}\right> &= \frac{\beta}{2} \left<\left(\int_{x_0}^{x} \mathrm{d}x^\prime \delta f(x') \right)^2\right>\\
	&= \beta \int_{x_0}^{x}  \text{d}x^\prime \int_{x_0}^{x^\prime}  \text{d}x^{\prime\prime} \left<\delta f(x^{\prime\prime}) \delta f (x^{\prime})\right> \; ,\\
\end{align*}
$$
...
$$
\begin{align}
 {\it{\Gamma}}(x) &= \frac{1}{v}\frac{\mathrm{d} W_\text{diss}}{\mathrm{d} x} \\
&= \beta \int_{0}^{t(x)} \text{d}\tau \left<\delta f(t(x))\delta f(\tau)\right>  \; ,
\end{align}
$$
