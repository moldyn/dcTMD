...
@pytest.fixture
def traj_feature_set2d(filenames_2d):
    return FeatureSet(filenames=filenames_2d, verbose=False)


def test_fill_array2d(traj_feature_set2d, feature_array_3d):
    """
    Test that fill_array loads data into an array with
    the expected shape and values.
    """
...
def test_get_filenames_missing_file(tmp_path, filenames_2d):
    """
    Test that _get_filenames raises FileNotFoundError when a file is missing.
    """
    # Create a temporary directory
    d = Path(TEST_FILE_DIR) / 'temp_data'
    d.mkdir(exist_ok=True)
    # Create one matching filename in temporary directory
    existing = d / filenames_2d[0]
    existing.touch()

    # wildcard pointing into temporary directory
    wildcard = str(d / 'feature_2D_{}.txt')

    # Expect FileNotFoundError because files are missing
...
def test_shape_mismatch():
    ...
    # The data from file1 should match,
    # while file2 is skipped because the shape does not match.
...
def test_fill_array_file_not_found():
    """
    Test if trying loading a non-existent file raises a ValueError.
    """
...
def test_verbose_output(filenames_2d, capsys):
    """
    Test that verbose output is printed when verbose=True.
    """
    f1 = filenames_2d[0]
    tfs = FeatureSet(filenames=filenames_2d, verbose=True)
    tfs.fill_array()

    captured = capsys.readouterr().out

    assert 'Loaded filenames:' in captured
    assert f'Using {f1}' in captured
    assert 'with shape' in captured
    assert f'Reading file {f1}' in captured
